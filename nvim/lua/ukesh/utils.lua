...
function M.hexToHSL(hex)
  ...
  -- return math.floor(h + 0.5) .. " " .. math.floor(s + 0.5) .. "% " .. math.floor(l + 0.5) .. "%"
end
...
function M.replaceHexWithHSL()
  ...
  -- Replace hex code patterns with HSL values
...
function M.visualHexToHSL()
  -- Get the visual selection directly
  local selected_text = vim.fn.getreg("")

  -- Replace hex codes with HSL using an external command (e.g., sed)
  local command = string.format("echo '%s' | sed 's/#[0-9a-fA-F]\\{6\\}/%s/g'", selected_text, function(hex)
    return M.hexToHSL(hex)
  end)
  ...
  vim.fn.feedkeys("gv") -- Reselect the last visual selection
end
